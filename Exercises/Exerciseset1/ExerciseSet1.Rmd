...
**Your Answers here:**
<br> 1. In the dataSet1.R file, stock price of Apple is downloaded in the line `r tmpdata<-as.matrix(try(getSymbols(Symbols=ticker_to_get,from = startDate,auto.assign=FALSE)))` when `ticker_to_get` gets the value `r AAPL`
Then daily returns are computed from the value of successive stock prices `StockReturns= ifelse(head(StockPrices,-1)!=0, (tail(StockPrices,-1)-head(StockPrices,-1))/head(StockPrices,-1),0)`
<br>
<br>
<br>
